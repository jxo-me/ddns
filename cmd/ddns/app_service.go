...
// AppService is the main service that runs when no command lines flags are passed to ddns
...
func (s *AppService) handleConfigUpdate(c config.Root) {
	...
	if c.Resolver.Enabled {
		...
		s.serviceManager.Add(service)
		activeServices[service.String()] = struct{}{}
	...
	for _, service := range s.serviceManager.Services() {
		if _, ok := activeServices[service.String()]; !ok {
			s.serviceManager.Remove(service.String())
		}
	}
}
