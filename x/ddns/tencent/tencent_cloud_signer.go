...
func TencentCloudSigner(secretId string, secretKey string, r *http.Request, action string, payload string) {
	...
	service := "dnspod"
	host := util.WriteString(service, ".tencentcloudapi.com")
	timestamp := time.Now().Unix()
	timestampStr := strconv.FormatInt(timestamp, 10)

	// step 1: build canonical request string
	canonicalHeaders := util.WriteString("content-type:application/json\nhost:", host, "\nx-tc-action:", strings.ToLower(action), "\n")
	signedHeaders := "content-type;host;x-tc-action"
	hashedRequestPayload := sha256hex(payload)
	canonicalRequest := util.WriteString("POST\n/\n\n", canonicalHeaders, "\n", signedHeaders, "\n", hashedRequestPayload)

	// step 2: build string to sign
	date := time.Unix(timestamp, 0).UTC().Format("2006-01-02")
	credentialScope := util.WriteString(date, "/", service, "/tc3_request")
	hashedCanonicalRequest := sha256hex(canonicalRequest)
	string2sign := util.WriteString(algorithm, "\n", timestampStr, "\n", credentialScope, "\n", hashedCanonicalRequest)

	// step 3: sign string
	secretDate := tencentCloudHmacsha256(date, util.WriteString("TC3", secretKey))
	...
	authorization := util.WriteString(algorithm, " Credential=", secretId, "/", credentialScope, ", SignedHeaders=", signedHeaders, ", Signature=", signature)
