...
var (
	...
	DefaultUnixConfigLocation = "/usr/local/etc/ddns"

	// DefaultUnixLogLocation is the primary location to find log files
	DefaultUnixLogLocation = "/var/log/ddns"
...
// If none of the combinations DefaultConfigSearchDirectories() and DefaultConfigFiles
//
//	contain a config file, return empty string.
...
var configuration FileSettings
...
//
//	if the value of "config" flag changes will read the new config file
func ReadConfigFile(c *cli.Context, log *zerolog.Logger) (settings *FileSettings, warnings string, err error) {
	...
	if err != nil {
		// If it does not exist and config file was not specifically specified, then return ErrNoConfigFile found.
